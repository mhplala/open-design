...
// subpath (NEXT_PUBLIC_OD_BASE_PATH=/design), AI-emitted absolute URLs
// need that prefix — `<base href>` only affects relative URLs, so we
// bake the absolute prefix into the prompt examples themselves. Read
// the env directly here (rather than importing the BASE_PATH helper
// from apps/web/src/runtime) because this file lives in the contracts
// package, which can't reach into the web app's source tree.
// Next.js inlines NEXT_PUBLIC_* at build time, so the value lands in
// every bundle that imports this string.
const BASE_PATH =
  (typeof process !== 'undefined' &&
    process.env &&
    process.env.NEXT_PUBLIC_OD_BASE_PATH) ||
  '';
