...
export function parseRoute(pathname: string): Route {
  // Strip the deployment basePath (e.g. /design) before matching segments,
  // so /design/projects/abc parses the same as /projects/abc.
...
export function buildPath(route: Route): string {
  // Wrap the route path with the deployment basePath once at the boundary,
  // so every caller-facing URL we hand to history.pushState carries the
  // /design prefix (when deployed under one).
  return withBasePath(buildRoutePath(route));
}

function buildRoutePath(route: Route): string {
  if (route.kind === 'home') {
