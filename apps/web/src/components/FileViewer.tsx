...
function HtmlViewer({
  ...
  // shouldUrlLoadHtmlPreview only knows about 'preview' | 'source'; collapse
  // our extended 'edit' mode to 'preview' for this decision (in edit mode
  // we always need our srcDoc shim, so URL-load is suppressed anyway —
  // useUrlLoadPreview later short-circuits when commentMode/edit are on).
  const useUrlLoadPreview = shouldUrlLoadHtmlPreview({
    mode: mode === 'edit' ? 'preview' : mode,
    isDeck: effectiveDeck,
    commentMode: boardMode || mode === 'edit',
    forceInline,
  });
  ...
  const srcDoc = useMemo(
    () => (previewSource ? buildSrcdoc(previewSource, {
      ...
      commentBridge: boardMode && mode !== 'edit',
      editMode: mode === 'edit',
    }) : ''),
    [previewSource, effectiveDeck, projectId, file.name, previewStateKey, boardMode, mode],
  );
    ...
    <div className="viewer html-viewer">
      ...
      >
        {source === null ? (
          ...
          // Edit mode: iframe + side Inspector. The iframe shares srcDoc
          // with preview mode but the shim is injected because the useMemo
          // above flips editMode on. Comment overlays are intentionally
          // suppressed in edit mode — selecting an element for editing
          // and writing a comment on it are conflicting gestures.
          <>
            <div
              style={{
                flex: '1 1 auto',
          ...
          // 'preview' mode (default) — upstream's comment-preview overlay.
