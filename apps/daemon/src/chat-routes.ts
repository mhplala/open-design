...
export function registerChatRoutes(app: Express, ctx: RegisterChatRoutesDeps) {
  ...
  app.post('/api/proxy/anthropic/stream', async (req, res) => {
    ...
    // MiMo (and other Anthropic-compat upstreams routed through this
    // endpoint) advertise much larger output budgets than the 8k default —
    // MiMo v2.5-pro: 128k. Honor the client's requested maxTokens, else
    // fall back to a model-aware ceiling. Extra logging captures system /
    // user payload sizes so truncation vs latency is diagnosable from the
    // daemon log alone.
    ...
    const userLen = Array.isArray(messages)
      ...
      : 0;
    console.log(
      `[proxy:anthropic] ${req.method} ${validated.parsed!.hostname} model=${model} miMo=${isMiMo} ` +
        `maxTokens=${effectiveMaxTokens} sysChars=${sysLen} userChars=${userLen}`,
    );
